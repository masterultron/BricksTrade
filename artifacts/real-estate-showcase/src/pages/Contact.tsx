...
export default function Contact() {
  ...
  return (
    <div className="bg-[hsl(var(--background))]">
        ...
        <div className="mx-auto max-w-[1400px]">
          ...
          <div className="mt-20 grid gap-16 border-t border-[hsl(var(--foreground)/.2)] pt-8 md:grid-cols-[.75fr_1.25fr] md:gap-28">
            <div className="space-y-10">
              ...
              <div>
                <p className="mono mb-4 text-[hsl(var(--muted-foreground))]">Follow along</p>
                <div className="flex gap-5">
                  <a href="https://www.instagram.com" target="_blank" rel="noreferrer" className="transition-colors hover:text-[hsl(var(--accent-orange))]" data-testid="link-instagram" aria-label="Instagram"><SiInstagram size={19} /></a>
                  <a href="https://www.facebook.com" target="_blank" rel="noreferrer" className="transition-colors hover:text-[hsl(var(--accent-orange))]" data-testid="link-facebook" aria-label="Facebook"><SiFacebook size={19} /></a>
                  <a href="https://www.tiktok.com" target="_blank" rel="noreferrer" className="transition-colors hover:text-[hsl(var(--accent-orange))]" data-testid="link-tiktok" aria-label="TikTok"><SiTiktok size={18} /></a>
                  <a href="https://www.linkedin.com" target="_blank" rel="noreferrer" className="transition-colors hover:text-[hsl(var(--accent-orange))]" data-testid="link-linkedin" aria-label="LinkedIn"><Linkedin size={19} /></a>
                </div>
              </div>
              ...
              <p className="mono mb-7 text-[hsl(var(--secondary))]">Tell us about the place</p>
               <AnimatePresence mode="wait">
                {!sent ? (
                  ...
                  <motion.div key="success" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="border border-[hsl(var(--secondary)/.35)] bg-[hsl(var(--muted))] p-8 md:p-12" data-testid="status-contact-success">
                    <div className="mb-8 flex h-12 w-12 items-center justify-center rounded-full bg-[hsl(var(--secondary))] text-[hsl(var(--secondary-foreground))]"><Check size={22} /></div>
                    <p className="mono mb-5 text-[hsl(var(--secondary))]">Message received</p>
                    <h2 className="display text-5xl leading-none">We’ll be in touch<br /><em>shortly.</em></h2>
                    <p className="mt-7 max-w-sm text-sm leading-6 text-[hsl(var(--muted-foreground))]">Thank you for starting the conversation. Our team usually replies within one working day.</p>
                    <button onClick={() => setSent(false)} className="mt-8 text-xs uppercase tracking-[.14em] underline underline-offset-4" data-testid="button-send-another">Send another message</button>
                  </motion.div>
                )}
               </AnimatePresence>
            </div>
      ...
      <section className="bg-[hsl(var(--secondary))] px-5 py-16 text-[hsl(var(--background))] md:px-10 md:py-20">
        <div className="mx-auto flex max-w-[1400px] flex-col justify-between gap-5 md:flex-row md:items-center">
          <p className="display text-4xl md:text-5xl">Good places start with<br /><em className="text-[hsl(var(--accent))]">good questions.</em></p>
          <div className="flex gap-8 text-sm opacity-80">
            <span className="flex items-center gap-2"><MapPin size={15} /> Irama Plaza, Gwarinpa</span>
            <span className="flex items-center gap-2"><Phone size={15} /> By appointment</span>
          </div>
        </div>
      </section>
    </div>
